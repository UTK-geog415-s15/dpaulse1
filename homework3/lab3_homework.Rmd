...
```
  ...
  - Residual vs. Fitted  
The first plot helps in visualizing whether or not the residuals meet the constant variance assumption. The hope is that the spread of residuals is even across all modeled values. In this case, the residuals seem to display homoskedasticity.  
  - Normal Q-Q  
The second plot helps us determine if the residuals are normally distributed (or at least close enough). The normality assumption is commonly met if N, the number of observations, is large enough as well. From looking at the plot, I would say the residuals are close enough to being normally distributed.  
  - Scale-Location  
The third plot also helps visualize whether or not the residuals are related to the predicted values (and therefore meet the constant variance assumption). Again, these residuals appear to meet this criterion.  
...
```
  ...
  b. This is a fairly small dataset, but based upon the Residual vs. Fitted plot, I would say that the residuals are fairly homoskedastic.From the Normal Q-Q plot, I would question whether or not the residuals were close enough to the normal distribution and would probably want to check a more definite test. The points are indeed close to the line, but they are still not quite as close as I would personally like. Since this is a small dataset, only 17 observations, I don't even think we can rely upon the Central Limit Theorem and say N is large enough. From the Scale-Location plot, I would again say that there is fairly constant varaiance. Although, from this graphic, it appears there may be a slight patter that residuals tend to be smalller at higher predicted values. In the Residuals vs. Leverage plot, we see that most of the points have a fairly small leverage; however, points 12 and 17 had relatively high residuals (the residuals were standardized and they are close to two standard deviations away from zero). They also have a fairly high Cook's D value. Because of these higher values, these observations may be considered outliers.
